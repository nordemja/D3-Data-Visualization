...
d3.csv('data/disasters.csv')
  ...
  for(let i = 0; i<data.length; i++){
  	...
  	let cx = mapFromDomainToRange(0, 365, timelineX1, timelineX2, daysFromStart) //******* don't use 0, use mapFromDomainToRange. Hint- use 365 days and don't worry about leap years... it is ok
  	let cy = getYearPosition(year, gapBetweenTimelines, startOfTimelinesY); //******* don't use 0, use getYearPosition
  	let radius = mapFromDomainToRange(0, 365, 5, 130, cost); //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
